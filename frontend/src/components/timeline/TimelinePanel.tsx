...
export function TimelinePanel({ events }: Props) {
  ...
  const togglePlanned = () => { if (showPlanned && !showActual) return; setShowPlanned((v) => !v); };
  const toggleActual  = () => { if (showActual && !showPlanned) return; setShowActual((v) => !v); };
              ...
              <div className={`pb-4 flex-1 ${i === events.length - 1 ? "pb-0" : ""}`}>
                ...
                {showPlanned && (
                  <div className="text-xs text-slate-600 mt-0.5">
                    {t("tl.planned", locale)} {formatUTC(event.planned_time)}
                  </div>
                )}
                {showActual && event.actual_time && (
                  <div className="text-xs text-green-600 mt-0.5">
                    {t("tl.actual2", locale)} {formatUTC(event.actual_time)}
                  </div>
                )}
              </div>
            </div>
